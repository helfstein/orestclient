...
namespace ORest.Expressions {
    internal static class ColumnExpression {
        ...
        public static IEnumerable<string> ExtractColumnNames<T, U>(this Expression<Func<T, IEnumerable<U>>> expression, ITypeCache typeCache) {
            var lambdaExpression = expression as LambdaExpression;
            if (lambdaExpression == null)
                throw Utils.NotSupportedExpression(expression);
        ...
        public static IEnumerable<string> ExtractColumnNames<T, U>(this Expression<Func<T, U>> expression, ITypeCache typeCache) {
            var lambdaExpression = expression as LambdaExpression;
            if (lambdaExpression == null)
                throw Utils.NotSupportedExpression(expression);
