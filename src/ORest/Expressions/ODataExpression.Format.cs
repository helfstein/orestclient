...
namespace ORest.Expressions {
    public partial class ODataExpression {
        ...
        private string FormatFunction(ExpressionContext context) {
            ...
            if (FunctionMapping.TryGetFunctionMapping(Function.FunctionName, Enumerable.Count<ODataExpression>(Function.Arguments), adapterVersion, out var mapping)) {
                ...
                var val = Enumerable.First<ODataExpression>(Function.Arguments);
                if (val.Value != null) {
        ...
        private string FormatAnyAllFunction(ExpressionContext context) {
            ...
            if (!Enumerable.Any<ODataExpression>(Function.Arguments) && string.Equals(Function.FunctionName, ODataLiteral.Any, StringComparison.OrdinalIgnoreCase)) {
                ...
                formattedArguments = $"{targetQualifier}:{FormatExpression(Enumerable.First<ODataExpression>(Function.Arguments), expressionContext)}";
            }
        ...
        private string FormatIsOfCastFunction(ExpressionContext context) {
            ...
            if (!ReferenceEquals(Enumerable.First<ODataExpression>(Function.Arguments), null) && !Enumerable.First<ODataExpression>(Function.Arguments).IsNull) {
                formattedArguments += FormatExpression(Enumerable.First<ODataExpression>(Function.Arguments), new ExpressionContext(context.Session));
                formattedArguments += ",";
            }
            formattedArguments += FormatExpression(Enumerable.Last<ODataExpression>(Function.Arguments), new ExpressionContext(context.Session));
        ...
        private string FormatEnumHasFlagFunction(ExpressionContext context) {
            var value = FormatExpression(Enumerable.First<ODataExpression>(Function.Arguments), new ExpressionContext(context.Session));
            return $"{FormatCallerReference()} has {value}";
        }

        private string FormatArrayIndexFunction(ExpressionContext context) {
            var propertyName =
                FormatExpression(Enumerable.First<ODataExpression>(Function.Arguments), new ExpressionContext(context.Session)).Trim('\'');
